...
The product removes monthly spreadsheet friction for a single user who imports CSV data from a Polish bank and needs useful spending categories quickly. The core promise is high first-pass auto-categorization quality with quick correction so category totals are trustworthy for budgeting decisions. This update prioritizes report correctness by replacing predefined budget-cycle selection with explicit start/end date selection (default: today minus 30 days to today).

## North star

**S-05: summary-date-range-window** — User can view the summary table for a chosen date window with a default range of today-30 days to today.

> The north star here means the smallest end-to-end slice whose successful delivery proves the core hypothesis of this roadmap update: explicit date-window reporting is the fastest path to trustworthy budget decisions.
...
| S-05 | summary-date-range-window | View summary totals for selected start/end dates (default: today-30d to today) | S-02 | US-01, FR-009, FR-010, FR-011 | ready |
...
| C | Core product completion | `S-03` / `S-04` / `S-05` | Parallel-ready capability expansion once import flow is stable, with S-05 focused on date-window reporting. |
...
### S-05: Summary date-range window

- **Outcome:** User can pick start and end dates (via datepicker) and see summary totals scoped to that window; default range is today minus 30 days through today.
- **Change ID:** summary-date-range-window
- **PRD refs:** US-01, FR-009, FR-010, FR-011
- **Prerequisites:** S-02
- **Parallel with:** S-03, S-04, S-06, S-07
- **Blockers:** —
- **Unknowns:** —
- **Risk:** Date boundary and timezone handling bugs can distort totals and erode trust in summary numbers.
- **Status:** ready
...
| S-05 | summary-date-range-window | Date-range summary with default last-30-days window | yes | Run `/10x-plan summary-date-range-window` |
